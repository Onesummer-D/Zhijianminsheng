...
def analyze_batch_rule_only(text):
    ...
    # 【修复】使用 classify_single，失败则手动遍历
    main_cat = None
    try:
        main_cat = rule.classify_single(text)
...
def build_batch_screening():
    with gr.Column():
        ...
        with gr.Row():
            ...
            # 右侧：台账区
            with gr.Column(scale=1):
                history_dd = gr.Dropdown(label="选择历史记录", choices=[], interactive=True)
                with gr.Row():
        ...
        def run_batch(file_obj, only_with_location, use_api):
            if file_obj is None:
                return pd.DataFrame(columns=result_table.headers), "请先上传Excel", None, gr.update(choices=[]), pd.DataFrame()
            
            try:
                df = pd.read_excel(file_obj.name if hasattr(file_obj, 'name') else file_obj)
            except Exception as e:
                return pd.DataFrame(columns=result_table.headers), f"读取失败: {e}", None, gr.update(choices=[]), pd.DataFrame()
            ...
            # 构造展示表（空表返回空DataFrame，不显示 1,2,3）
            if display_df.empty:
                show_df = pd.DataFrame(columns=["工单编号", "工单内容", "主要类别", "次要类别", "置信度", "点位标记", "重复次数", "命中核心词", "相似案例", "关联法条"])
            else:
...
with gr.Blocks(css=CUSTOM_CSS, title="智检民声 - 12345涉检线索智能筛查系统") as demo:
    ...
    with gr.Tabs(elem_classes=["hidden-tabs"]) as tabs:
        ...
        # --- Tab 0: 首页 ---
        with gr.TabItem("首页", elem_id="tab-home"):
        ...
        # --- Tab 1: 单条查询 ---
        with gr.TabItem("单条", elem_id="tab-single"):
            with gr.Row():
                with gr.Column(scale=1):
        ...
        # --- Tab 2: 批量筛查 ---
        with gr.TabItem("批量", elem_id="tab-batch"):
            with gr.Row():
                with gr.Column(scale=1):
    ...
    # --- 页面切换事件（必须在 with gr.Blocks 内部，但在 with gr.Tabs 外部）---
    btn_to_single.click(lambda: gr.update(selected=1), None, tabs)
    btn_to_batch.click( lambda: gr.update(selected=2), None, tabs)
    btn_back_1.click(   lambda: gr.update(selected=0), None, tabs)
    btn_back_2.click(   lambda: gr.update(selected=0), None, tabs)
