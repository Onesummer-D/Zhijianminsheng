...
def mock_classify(text):
    if not text:
        return {"type": "无法识别", "confidence": 0, "law": "无", "reason": "文本为空", "suggestion": "请输入投诉内容"}
    text = str(text).lower()
    if any(kw in text for kw in ["污染", "环境"]):
        return {"type": "公益诉讼", "confidence": 0.85, "law": "环境保护法第58条", "reason": "涉及环境污染线索", "suggestion": "建议移送公益诉讼部门"}
    elif any(kw in text for kw in ["欠薪", "工资"]):
        return {"type": "民事支持起诉", "confidence": 0.82, "law": "劳动合同法第30条", "reason": "涉及劳动者权益保护", "suggestion": "建议支持起诉"}
    elif any(kw in text for kw in ["诈骗", "盗窃"]):
        return {"type": "刑事犯罪", "confidence": 0.88, "law": "刑法相关条款", "reason": "涉嫌刑事犯罪", "suggestion": "建议移送刑事检察部门"}
    else:
        return {"type": "普通投诉", "confidence": 0.3, "law": "不涉及检察监督", "reason": "暂未发现涉检线索", "suggestion": "建议按普通信访处理"}
...
with gr.Blocks(title="智检民声 - 12345涉检线索筛查") as demo:
    gr.Markdown("# 智检民声")
    ...
    with gr.Tab("📁 批量筛查"):
        ...
        with gr.Row():
            with gr.Column(scale=1):
                file_input = gr.File(label="上传Excel", file_types=[".xlsx", ".xls"])
                btn_batch = gr.Button("🚀 开始筛查", variant="primary")
            with gr.Column(scale=2):
                output_table = gr.DataFrame(label="筛查结果预览（前5条）", interactive=False)
    ...
    with gr.Tab("🔍 单条查询"):
        ...
        with gr.Row():
            with gr.Column(scale=1):
                text_input = gr.Textbox(label="投诉内容", placeholder="请粘贴12345投诉原文...", lines=8)
                btn_single = gr.Button("🔎 分析", variant="primary")
...
if __name__ == "__main__":
    demo.launch(server_name="0.0.0.0", server_port=6009, share=False, show_error=True)
